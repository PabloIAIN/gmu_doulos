    ...
    if (req.method === 'POST') {
      ...
      if (miembros && Array.isArray(miembros)) {
        for (const m of miembros) {
          await sql`
            ...
            ON CONFLICT (id) DO UPDATE SET nombre=EXCLUDED.nombre,apellido=EXCLUDED.apellido,fecha_nacimiento=EXCLUDED.fecha_nacimiento,telefono=EXCLUDED.telefono,email=EXCLUDED.email,clase=EXCLUDED.clase,rol=EXCLUDED.rol,activo=EXCLUDED.activo,usuario=EXCLUDED.usuario,password_hash=EXCLUDED.password_hash,updated_at=NOW()`;
          resultados.miembros++;
        }
      }
      ...
      if (eventos && Array.isArray(eventos)) {
        for (const e of eventos) {
          await sql`
            ...
            ON CONFLICT (id) DO UPDATE SET titulo=EXCLUDED.titulo,descripcion=EXCLUDED.descripcion,fecha=EXCLUDED.fecha,hora=EXCLUDED.hora,ubicacion=EXCLUDED.ubicacion,tipo=EXCLUDED.tipo,updated_at=NOW()`;
          resultados.eventos++;
        }
      }
      ...
      if (unidades && Array.isArray(unidades)) {
        for (const u of unidades) {
          await sql`
            INSERT INTO unidades (id,nombre,descripcion,activo,fecha_creacion) VALUES (${u.id},${u.nombre},${u.descripcion||null},${u.activo!==undefined?u.activo:1},${u.fecha_creacion||new Date().toISOString()})
