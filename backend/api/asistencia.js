  ...
  try {
    switch (req.method) {
      case 'GET': {
        ...
        if (unidad_id && fecha) {
          // Asistencia de una unidad en una fecha específica
          const result = await sql`
            SELECT a.*, m.nombre, m.apellido
            FROM asistencia a
            JOIN miembros m ON m.id = a.miembro_id
            WHERE a.unidad_id = ${unidad_id} AND a.fecha = ${fecha}
            ORDER BY m.nombre
          `;
        ...
        // Todas las asistencias (últimas 100)
        const result = await sql`
          SELECT a.*, m.nombre, m.apellido, u.nombre as unidad_nombre
          FROM asistencia a
          JOIN miembros m ON m.id = a.miembro_id
          JOIN unidades u ON u.id = a.unidad_id
          ORDER BY a.fecha DESC
      ...
      case 'POST': {
        ...
        if (!id || !unidad_id || !miembro_id || !fecha) {
          return res.status(400).json({ error: 'id, unidad_id, miembro_id y fecha son requeridos' });
        }
      ...
      // ── POST masivo para registrar asistencia de toda una unidad ──
      case 'PUT': {
        const { registros } = req.body;
        if (!registros || !Array.isArray(registros)) {
          return res.status(400).json({ error: 'registros (array) es requerido' });
        }
